...
#include <cstdint>
	...
	uintptr_t logInt() const { return T::logint(); }

	bool kernelMem() const { return T::kernelmem(); }
	
	bool imageMem() const { return T::imagemem(); }

	bool can() const { return T::canonical(); }		

	T* log() const { return T::logical(); }

	uintptr_t physInt() const { return T::physint(); }

	const uint64_t PHYS() { return T::phys(); }

	const uint64_t SIZE() { return T::size(); }

	const uint64_t VIRT() { return T::virt(); }  
protected:
	//uint32_t ptr;
	uint64_t ptr;
};
